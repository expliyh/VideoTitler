# VideoTitler

从指定目录批量读取视频，截取第 X 帧后可选择百度 OCR 流程，或使用 DeepSeek 视觉多模态直接识别游戏任务画面中的章标题、节标题、任务简述和详细任务内容，再重命名为 `序号-标题`。

## 安装

在项目目录下：

```powershell
.\.venv\Scripts\python.exe -m pip install -r requirements.txt
```

另外需要安装 `ffmpeg` 并确保可在命令行直接执行 `ffmpeg`（已加入 PATH）。

## 运行

```powershell
.\.venv\Scripts\python.exe .\main.py
```

## 使用说明

- 在 GUI 中选择“视频目录”，点击“扫描”
- 设置“第 X 帧(从1开始)”
- OCR 模式填写百度 OCR 的 `API Key / Secret Key`（从百度智能云控制台获取）；视觉多模态模式只需要 DeepSeek API Key
- 填写 DeepSeek 的 `API Key`
- 在“识别模式”中选择 OCR 或“视觉多模态”，点击“开始处理”

提示：
- 勾选“仅预览(不改名)”可先检查提取结果
- 勾选“保存密钥到本地 config.json”会把密钥明文保存在本项目目录，请自行注意安全
- “密钥/设置”中可分别修改 OCR 与视觉模式的 DeepSeek Prompt；OCR 的 `User Prompt 模板`支持占位符 `{ocr_text}`
- 在 “OCR/日志” 页可手动编辑 OCR 结果/标题，并对单条视频重新生成标题或重命名（失败的也可以补救）
- OCR 默认使用百度“高精度”(`accurate_basic`)，可在主界面下拉切换为通用(`general_basic`)
- 视觉多模态模式默认使用 `deepseek-v4-flash`，会直接把当前帧图片发送给 DeepSeek，并显示章标题、节标题、任务简述和详细任务内容
- 如果先勾选“仅预览(不改名)”跑一遍，可在确认/编辑标题后点击“重命名全部”一次性执行改名

## 前端桌面端（Electron + React）

已新增 Monorepo 结构：

- `apps/desktop/`：Electron 主进程、预加载脚本、React 渲染进程
- `packages/core/`：与 UI 无关的业务逻辑（目录扫描过滤、标题提取、重命名规则）

### 运行脚本

```bash
pnpm install
pnpm dev
pnpm build
pnpm dist
```

### 当前已迁移 UI 功能点

- 目录扫描
- 帧号设置（批量更新）
- OCR 结果编辑
- 视觉多模态任务信息识别与编辑
- 单条重命名
- 批量重命名
- 日志查看
